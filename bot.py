...
class Game:
    ...
    async def start_day(self, kills: list[Player] = None, origin=None):
        if kills is None:
            kills = []
        ...
        if not global_vars.game.days[-1].riot_active:
            if (global_vars.game.show_tally):
                ...
                last_vote_message = None if not has_had_multiple_votes else (
                    await global_vars.channel.fetch_message(self.votes[-1].announcements[0]) if self.votes and self.votes[
...
class Vote:
    ...
    def __init__(self, nominee, nominator):
        ...
        self.nominator: Player = nominator
        ...
        self.votes: int = 0
        self.voted: list[Player] = []
        self.history = []
        self.announcements = []
        self.presetVotes = {}
        self.values: dict[Player, tuple[int, int]] = {person: (0, 1) for person in self.order}
        self.majority: float = 0.0
    ...
    async def end_vote(self):
        ...
        if dies:
            if aboutToDie is not None and aboutToDie[0
...
class Player:
    ...
    async def kill(self, suppress=False, force=False):
        dies = True
        on_death_characters = sorted([person.character for person in global_vars.game.seatingOrder if isinstance(person.character,
                                                                                                     DeathModifier)],
                                     key=lambda c: c.on_death_priority())
    ...
    async def message(self, frm, content, jump):
        ...
        try:
            ...
            logger.info(
                "could not send message to {}; it is {} characters long; error {}".format(self.nick, len(content),
                                                                                          e.text))
            return
        ...
        # await safe_send(global_vars.channel,'**{}** > **{}**'.format(frm.nick, self.nick))
...
class Riot(Demon, NominationModifier):
    ...
    async def on_nomination(self, nominee, nominator, proceed):
        ...
        soldier_jinx = nominator and nominee and not nominee.character.is_poisoned and has_ability(nominator.character,
                                                                                                   Riot) and has_ability(
            nominee.character, Soldier)
...
async def update_presence(client):
    ...
    if global_vars.game is NULL_GAME:
        ...
        whisper_state = "to " + global_vars.game.whisper_mode if global_vars.game.days[-1].isPms and global_vars.game.whisper_mode != WhisperMode.ALL else \
        clopen[global_vars.game.days[-1].isPms]
...
async def load(fileName):
    ...
    global_vars.game = Game([], None, Script([]))
    for obj in objects:
        ...
        with open(obj + "_" + fileName, "rb") as file:
            if obj == "seatingOrderMessage":
                ...
                setattr(global_vars.game, obj, msg)
            else:
                setattr(global_vars.game, obj, dill.load(file))

    return global_vars.game
...
async def safe_send(target: Optional[discord.abc.Messageable], msg: str):
    ...
    if message.guild is None:
        ...
        if message.content.startswith(prefixes):
            ...
            # Views relevant information about a player
                ...
                for memb in global_vars.gamemaster_role.members:
                    await safe_send(memb, "Atheist game is set to {} by {}".format(global_vars.game.script.isAtheist,
                                                                                   message.author.display_name))
                pass
            # Views the grimoire
            ...
            # Nominates
                ...
                banshee_ability_of_player = the_ability(nominator_player.character,
                                                        Banshee) if nominator_player else None
            ...
            # Help dialogue
                ...
                if global_vars.gamemaster_role in global_vars.server.get_member(message.author.id).roles:
                    if argument == "":
                        ...
                        embed.add_field(
                            name="whispermode", value="modifies whisper mode to 'all', 'neighbors', or 'storytellers'",
                            inline=False
                        )
                        embed.add_field(
                            name="close",
                ...
                embed.add_field(
                    name="Bot Questions?",
                    value="Discuss or provide feedback at https://github.com/fiveofswords/botc-bot", inline=False
                )
                # this is a rare place that should use .send -- it is sending an embed and safe_send is not set up to split if needed
                await message.author.send(embed=embed)
                return
...
async def on_message_edit(before, after):
    ...
    if after.channel == global_vars.channel and before.pinned == False and after.pinned == True:
        ...
        if "nominate " in after.content.lower():
            ...
            banshee_ability_of_player = the_ability(message_author_player.character,
    ...
    # On unpin
    elif after.channel == global_vars.channel and before.pinned == True and after.pinned == False:
...
NULL_GAME: Game = Game(seatingOrder=[], seatingOrderMessage=0, script=[], skip_storytellers=True)
