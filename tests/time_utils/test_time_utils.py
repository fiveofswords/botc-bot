...
class TestParseDeadline:
    ...
    def test_time_in_future_sets_same_day(self):
        ...
        assert expected == actual, "The parsed deadline does not match the expected value."
    ...
    def test_invalid_string_returns_none(self):
        input_deadline = "1223"
        assert parse_deadline(input_deadline) is None


class TestConvertToTimeDelta:
    def test_accepts_only_hours(self):
        input_deadline = "+2h"
        assert _convert_to_timedelta(input_deadline) == timedelta(hours=2, minutes=0)

    def test_accepts_only_minute(self):
        input_deadline = "+30m"
        assert _convert_to_timedelta(input_deadline) == timedelta(hours=0, minutes=30)

    def test_accepts_hours_and_minutes(self):
        input_deadline = "+3h30m"
        assert _convert_to_timedelta(input_deadline) == timedelta(hours=3, minutes=30)
    ...
    def test_missing_prefix_returns_none(self):
        input_deadline = "2h"
        assert _convert_to_timedelta(input_deadline) is None
...
class TestRoundDatetimeToHalfHour:
    def test_rounds_up_to_hour(self):
        ...
        assert _round_datetime_to_nearest_half_hour(input_datetime) == expected
