...
import discord
import global_vars
from bot_impl import on_message, on_message_edit, on_member_update, Game, Script, Vote, Day, NULL_GAME
...
async def test_welcome_command(mock_discord_setup, setup_test_game):
    """Test the welcome command with comprehensive coverage of the implementation."""
    ...
    # Set up storyteller and target player
    storyteller = mock_discord_setup['members']['storyteller']
    alice = mock_discord_setup['members']['alice']
    
    # Make storyteller a gamemaster
    storyteller.roles = [mock_discord_setup['roles']['gamemaster']]
    
    # Mock server get_member to return storyteller
    mock_discord_setup['guild'].get_member = MagicMock(return_value=storyteller)
    
    # Create the welcome command message
    welcome_message = MockMessage(
        id=130,
        content="@welcome alice",
        channel=st_dm_channel,
        author=storyteller,
        guild=None  # DM has no guild
    )

    # Mock client object with user id
    mock_client = MagicMock()
    mock_client.user = MagicMock()
    mock_client.user.id = 999
    mock_client.user.display_name = "Blood on the Clocktower Bot"
    
    # Set up server properties
    mock_discord_setup['guild'].name = "Test Server"
    
    # Mock ST channel
    mock_st_channel = MockChannel(12345, f"st-alice")
    
    # Setup storytellers list for the welcome message formatting
    mock_discord_setup['roles']['gamemaster'].members = [storyteller]

    # Create comprehensive test with all necessary mocks
    with patch('bot_impl.select_player', return_value=setup_test_game['players']['alice']), \
         patch('bot_impl.safe_send', new_callable=AsyncMock) as mock_safe_send, \
         patch('bot_client.client', mock_client), \
         patch('bot_impl.client', mock_client), \
         patch('model.settings.game_settings.GameSettings.load') as mock_settings_load, \
         patch('bot_impl.client.get_channel', return_value=None), \
         patch('bot_impl.ChannelManager') as mock_channel_manager_class, \
         patch('bot_impl.update_presence', new_callable=AsyncMock), \
         patch('bot_impl.backup'):
        
        # Setup game settings mock
        mock_game_settings = MagicMock()
        mock_game_settings.get_st_channel.return_value = None 
        mock_settings_load.return_value = mock_game_settings
        
        # Setup channel manager
        mock_channel_manager = MagicMock()
        mock_channel_manager.create_channel = AsyncMock(return_value=mock_st_channel)
        mock_channel_manager_class.return_value = mock_channel_manager
        
        # Process the welcome command
        await on_message(welcome_message)
        
        # Call args have been checked and fixed
        
        # Verify welcome message was sent to the player
        player_welcome_msg_found = False
        for call in mock_safe_send.call_args_list:
            args, _ = call
            # Check if recipient is either the Player object or player.user
            alice_player = setup_test_game['players']['alice']
            alice_user = alice_player.user
            
            if args[0] == alice_user or args[0] == alice_player:
                player_welcome_msg_found = True
                welcome_msg = args[1]
                # Verify welcome message contains expected elements
                assert "Hello" in welcome_msg or "Welcome" in welcome_msg
                assert "Blood on the Clocktower" in welcome_msg
                assert "Storyteller channel" in welcome_msg or "channel" in welcome_msg
                assert "@pm" in welcome_msg or "pm" in welcome_msg
                assert "@help" in welcome_msg or "help" in welcome_msg
                break
        assert player_welcome_msg_found, "No welcome message was sent to the player"
        
        # Verify confirmation message was sent to the storyteller
        st_confirm_msg_found = False
        for call in mock_safe_send.call_args_list:
            args, _ = call
            if args[0] == storyteller:
                if "Welcome" in args[1] or "success" in args[1]:
                    st_confirm_msg_found = True
                    break
        assert st_confirm_msg_found, "No confirmation message was sent to the storyteller"
        
        # Verify channel check/creation was attempted
        assert mock_game_settings.get_st_channel.called
        assert mock_channel_manager.create_channel.called
