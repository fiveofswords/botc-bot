...
from tests.fixtures.discord_mocks import mock_discord_setup, MockChannel, MockMember, MockMessage
...
def disable_backup():
    ...
    with patch('utils.game_utils.backup', return_value=None) as mock_backup:
        with patch('utils.game_utils.remove_backup', return_value=None) as mock_remove:
...
def common_patches():
    ...
    patches = [
        patch('utils.game_utils.backup'),  # Completely disable backup function
        patch('utils.game_utils.remove_backup'),  # Disable backup removal function
...
async def test_on_message_inactive_game(mock_discord_setup):
    ...
    # Process the message
    with patch('utils.game_utils.backup'):  # Ignore backup calls
...
async def test_complete_voting_workflow(mock_discord_setup, setup_test_game):
    ...
    # Process Bob's vote
    with patch('utils.player_utils.get_player', return_value=setup_test_game['players']['bob']):
        with patch('utils.game_utils.backup', return_value=None), patch('utils.message_utils.safe_send',
                                                                new_callable=AsyncMock):
...
async def test_on_ready(mock_discord_setup):
    ...
    # Use the full bot setup patches
...
async def test_on_ready_with_backup_file(mock_discord_setup, setup_test_game):
    ...
    # Use the full bot setup patches with backup loading
    patches = full_bot_setup_patches_combined(mock_discord_setup, with_backup=True, backup_game=setup_test_game['game'])
    with ExitStack() as stack:
        mock_load = None
        for p in patches:
