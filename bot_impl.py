    ...
    if message.guild is None:
        ...
        if message.content.startswith(config.PREFIXES):
            ...
            # Presets a vote
                ...
                # Migrated to commands/voting_commands.presetvote_command
                await message_utils.safe_send(message.author, "Preset vote handling has been migrated to the commands system.")
                return

            # Cancels a preset vote
            elif command == "cancelpreset" or command == "cancelprevote":

                # Migrated to commands/voting_commands.cancelprevote_command
                await message_utils.safe_send(message.author, "Cancel prevote handling has been migrated to the commands system.")
                return
