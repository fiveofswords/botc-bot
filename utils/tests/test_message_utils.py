...
from utils.message_utils import _split_text, safe_send, safe_send_dm
...
class TestSafeSendDM:
    ...
    @pytest.fixture(autouse=True)
    def setup_test(self):
        self.user = mock.AsyncMock(spec=discord.User)
        self.dm_channel = mock.AsyncMock(spec=discord.DMChannel)
    ...
    async def test_dm_channel_exists(self, mock_safe_send):
        ...
        mock_safe_send.return_value = mock.AsyncMock(spec=discord.Message)
    ...
    async def test_handle_http_exception(self, mock_logger):
        ...
        self.dm_channel.send.side_effect = discord.HTTPException(response=mock.Mock(), message="Error")
